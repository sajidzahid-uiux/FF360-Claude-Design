...
export interface WeekGridSkeletonProps {
  className?: string;
}

export function WeekGridSkeleton({ className }: WeekGridSkeletonProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      >
        {Array.from({ length: GRID_COLUMNS }).map((_, index) => {
          const isLastInRow = index === GRID_COLUMNS - 1;
          const chipCount = ((index + 1) * 5) % (CHIPS_PER_CELL + 1);
