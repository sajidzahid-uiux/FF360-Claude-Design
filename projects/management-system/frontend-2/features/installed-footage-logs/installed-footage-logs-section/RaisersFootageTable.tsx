...
import { formatLogDate, memberLabel } from "./footageLogFormat";
import type { FootageLogTableProps } from "./footageLogTableTypes";

export function RaisersFootageTable({
  rows,
  isLoading,
  isError,
  disabled,
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {rows.map((row) => {
            if (row.log_type !== "raisers") return null;
            const mod = canModifyRow(row);
            return (
              <tr key={`raisers:${row.id}`} className="border-t align-top">
                <td className="px-4 py-5">{memberLabel(row)}</td>
                <td className="px-4 py-5">{formatLogDate(row.date)}</td>
                <td className="px-4 py-5">{row.quantity}</td>
