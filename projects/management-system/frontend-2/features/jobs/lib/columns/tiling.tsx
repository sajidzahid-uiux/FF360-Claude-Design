...
function TilingJobNameCell({ job }: { job: Job }) {
...
export function getTilingJobColumns(
  onShowMore: (id: number, isArchived: boolean) => void,
  onTrash: (id: number) => void,
  onUnarchive: (id: number) => void,
  onArchive: (id: number) => void,
  ...
  builder.addColumn({
    ...
    header: "Job Name",
    cell: ({ row }) => <TilingJobNameCell job={row.original} />,
  });
  ...
  builder.addColumn({
    ...
    cell: ({ row }) =>
      formatMaterialStatusLabel(row.original.material_status) || null,
  });
